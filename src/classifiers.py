...
class Classifier:
    def __init__(
        ...
        self.train_subjects, self.test_subjects = model_selection.train_test_split(
            self.has_node_subjects,
            train_size=0.5,
            test_size=0.2,
            stratify=self.has_node_subjects,
        )
    ...
    def set_classifiers(self, classifier_path, dataowner, hasG_hide: StellarGraph):
        ...
        self.hasG_node_gen = GraphSAGENodeGenerator(
            self.hasG, self.batch_size, self.num_samples
        )
        self.all_train_gen = self.hasG_node_gen.flow(
            self.train_subjects.index, self.train_targets, shuffle=True
        )
        self.all_test_gen = self.hasG_node_gen.flow(
    ...
    def build_classifier(self, fillG_node_gen):
        graphsage_model = GraphSAGE(
            layer_sizes=config.classifier_layer_sizes,
            generator=fillG_node_gen,
            n_samples=config.num_samples,
        )
        x_inp, x_out = graphsage_model.in_out_tensors()
        prediction = layers.Dense(len(self.train_targets[0]), activation="softmax")(
            x_out
        )
    ...
    def eval_pred_Gnew(
        ...
        adj,
        acc_path,
        impaired_graph: StellarGraph,
        org_graph: StellarGraph,
        test_flag=False,
        save_flag=False,
        global_task=None,
    ):
        pred_missing, pred_feats = self.pred_missing_neigh(
            generator_model, all_feat, edges, adj
        )

        if test_flag == True:
            fill_nodes, fill_G = mending_graph.fill_graph(
                impaired_graph, org_graph, pred_missing, pred_feats, self.feat_shape
            )
            fillG_node_gen = GraphSAGENodeGenerator(
                fill_G, self.batch_size, self.num_samples
            )
            fill_train_gen = fillG_node_gen.flow(
                self.train_subjects.index, self.train_targets, shuffle=True
            )
            ...
            if os.path.isfile(self.classifier_path[-1] + "locSagePC.h5") == False:
                history = self.locSagePC.fit(
                    fill_train_gen,
                    epochs=config.epoch_classifier,
                    verbose=2,
                    shuffle=False,
                )

                if save_flag:
                    self.locSagePC.save_weights(
                        self.classifier_path[-1] + "locSagePC.h5"
                    )
            else:
                self.locSagePC.load_weights(self.classifier_path[-1] + "locSagePC.h5")
            if global_task.test_only_gen is not None:
                self.test_global(
                    global_task, self.locSagePC, acc_path, "LocSagePlusC", ""
                )
