...
class LocalOwner:
    def __init__(
        ...
        self.neighgen = LocalSage_Plus(
            feat_shape=self.feat_shape,
            node_len=len(self.all_ids),
        ...
        self.optimizer = optim.Adam(
            self.neighgen.parameters(), lr=config.lr, weight_decay=config.weight_decay
        )
        if config.cuda:
    ...
    def wn_hide_subj(self):
        hide_len = int(
            (
                len(list(self.node_subjects.index))
                - len(list(self.train_subjects.index))
                - len(list(self.test_subjects.index))
            )
            * config.hidden_portion
        )
        could_hide_ids = self.node_subjects.index.difference(
            self.train_subjects.index
        ).difference(self.test_subjects.index)

        hide_ids = np.random.choice(could_hide_ids, hide_len, replace=False)
        return (
            self.node_subjects[
                self.node_subjects.index.difference(hide_ids)
                .difference(self.train_subjects.index)
                .difference(self.test_subjects.index)
            ],
            hide_ids,
        )
    ...
    def get_train_test_feat_targets(self):
        ...
        self.train_targets_subj = preprocessing.label_binarize(
            self.train_subjects, classes=self.all_classes
        )
        self.test_targets_subj = preprocessing.label_binarize(
            self.test_subjects, classes=self.all_classes
        )
        self.wn_hide_targets_subj = preprocessing.label_binarize(
            self.wn_hide_subjects, classes=self.all_classes
        )
        self.all_node_targets_subj = preprocessing.label_binarize(
            self.node_subjects, classes=self.all_classes
        )

        for id_i in self.all_ids:
            missing_ids = list(
                set(self.subG.neighbors(id_i)).difference(
                    list(self.hasG_hide.neighbors(id_i))
                )
            )
            ...
            if missing_len > 0:
                if len(missing_ids) <= self.num_pred:
                    zeros = np.zeros(
                        (max(0, self.num_pred - missing_len), self.feat_shape)
                    )
                    missing_feat_all = np.vstack(
                        (self.subG.node_features(missing_ids), zeros)
                    ).reshape((1, self.num_pred, self.feat_shape))
                else:
                    missing_feat_all = np.copy(
                        self.subG.node_features(missing_ids[: self.num_pred])
                    ).reshape((1, self.num_pred, self.feat_shape))
            else:
        ...
        self.all_targets_feat = np.asarray(self.all_targets_feat).reshape(
            (-1, self.num_pred, self.feat_shape)
        )
        self.all_targets_subj = np.asarray(self.all_node_targets_subj).reshape(
            (-1, self.num_classes)
        )
    ...
    def l2_feat(self, output, labels):
        ...
        return F.mse_loss(output, labels).float()
    ...
    def greedy_l2_feat(self, pred_feats, true_feats, pred_missing, true_missing):
        ...
        return (
            feat_loss.greedy_loss(pred_feats, true_feats, pred_missing, true_missing)
            .unsqueeze(0)
            .mean()
            .float()
        )
    ...
    def train_neighgen(self, epoch):
        ...
        input_adj = self.adj
        output_missing, output_feat, output_nc = self.neighgen(
            input_feat, input_edge, input_adj
        )
        output_missing = torch.flatten(output_missing)
        output_feat = output_feat.view(
            len(self.all_ids), self.num_pred, self.feat_shape
        )
        ...
        true_nc_label = torch.argmax(
            self.all_targets_subj[self.train_ilocs], dim=1
        ).view(-1)
        if config.cuda:
        ...
        self.neighgen.eval()
        val_missing, val_feat, val_nc = self.neighgen(
            self.all_feat, self.edges, self.adj
        )
        val_feat = val_feat.view(len(self.all_ids), self.num_pred, self.feat_shape)
        acc_val_missing = self.accuracy_missing(
            val_missing[self.train_ilocs], self.all_targets_missing[self.train_ilocs]
        )
        l2_val_feat = self.greedy_l2_feat(
            val_feat[self.train_ilocs],
            self.all_targets_feat[self.train_ilocs],
            val_missing[self.train_ilocs],
            self.all_targets_missing[self.train_ilocs],
        )
        acc_val_nc = self.accuracy(
            val_nc[self.train_ilocs], self.all_targets_subj[self.train_ilocs]
        )
    ...
    def load_model(self):
        if config.cuda:
            self.neighgen = torch.load(
                self.model_path[1], map_location=torch.device("cuda")
            )
        else:
            self.neighgen = torch.load(
                self.model_path[1], map_location=torch.device("cpu")
            )

    def evaluate_downstream(
        self, classifier: Classifier, test_flag=False, save_flag=False, global_task=None
    ):
        pred_missing, pred_feats = classifier.eval_pred_Gnew(
            generator_model=self.neighgen,
            impaired_graph=self.hasG_hide,
            org_graph=self.subG,
            all_feat=self.all_feat,
            edges=self.edges,
            adj=self.adj,
            acc_path=classifier.acc_path,
            test_flag=test_flag,
            save_flag=save_flag,
    ...
    def load_fed_model(self):
        if config.cuda:
            ...
            self.fed_model = torch.load(
                self.model_path[0], map_location=torch.device("cpu")
            )
