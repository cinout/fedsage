...
def set_up_system():
    if config.dataset == "cora":
        ...
        G, node_subjects = load_from_npz(
            config.root_path + "other_datasets/ms_academic.npz"
        )
    else:
    ...
    dataowner_list = []
    local_G, local_subj, local_target, local_nodes_ids = louvain_graph_cut(
        G, node_subjects
    )
    ...
    for owner_i in range(config.num_owners):
        do_i = DataOwner(
            do_id=owner_i,
            subG=local_G[owner_i],
            sub_ids=local_nodes_ids[owner_i],
    ...
    for owner_i in range(config.num_owners):
        ...
        local_classifier = Classifier(
            ...
            acc_path=do_i.test_acc_path,
            classifier_path=do_i.classifier_path,
            downstream_task_path=do_i.downstream_task_path,
        )

        local_gen = LocalOwner(
            do_id=owner_i,
            subG=do_i.hasG,
            node_subjects=do_i.has_subj,
            all_classes=all_classes,
            num_samples=config.num_samples,
            model_path=[do_i.fedgen_model_path, do_i.gen_model_path],
        )
        local_classifier.set_classifiers(
            classifier_path=do_i.classifier_path,
            dataowner=do_i,
    ...
    for do_i in range(config.num_owners):
        local_owners[do_i].evaluate_downstream(
            local_classifiers[do_i],
            test_flag=True,
            save_flag=True,
    ...
    train_fedSagePC(
        local_classifiers, local_owners, global_task, config.global_gen_nc_acc_file
    )
